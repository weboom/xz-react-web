...
export default class Welcome extends React.Component {
  ...
  public componentDidMount() {
    ...
    api
      ...
      .then((res : any) => {
        if (res && res.success) {
          this.setState({slideList: res.data.list})
        }
      })
    api
      .getMainMenu()
      .then((res : any) => {
        if (res && res.success) {
          this.setState({menuList: res.data.list})
        }
      })
  }
  ...
  public render() {
    return (
      <div className="page-welcome page-has-tabbar">
        ...
        {this.renderMenu()}
        <div className="mod-cate-box">
          <div className="mod-cate">
            <XzCategoryList
              categoryId={this.state.curCategoryId}
              onClickItem={(categoryId) => {
              this.setState({curCategoryId: categoryId})
            }}/>
          </div>
        </div>
